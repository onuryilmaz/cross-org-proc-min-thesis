...
% 1)Inductive Miner – infrequent (default) Applies noise filtering during mining. The threshold for noise filtering can be set using the slider ‘noise threshold’; 0.00 filters no noise, 1.00 filters noise maximally. For more information, 

%In this section, we introduce Inductive Miner - infrequent (IMi) by adding infrequent behaviour filters to all steps of IM. For each of the operational steps of IM it is described how infrequent behaviour affects the step and how distinguishing frequent and infrequent behaviour can be used to improve discovery of the 80\% model. In each recursion step, IMi first applies the steps of IM unaltered. Only if this fails and IM would return a flower model, the filters are applied. Frequencies of traces and events are ignored by IM but are taken into account by IMi in order to distinguish frequent and infrequent behaviour. In the operator and cut selection steps, two techniques are applied: filtering the directly-follows graph for infrequent edges and using a variant of the directly-follows graph for selection of !. Filters are added to base case detection to filter accumulated artifacts of filtering over recursions. In the following, k denotes a user-defined threshold value between 0 and 1 to separate frequent and infrequent behaviour. Filters on the operator and cut selection steps are described first, followed by filters on base cases, and filters on log splitting.
...
% 2)Inductive Miner – incompleteness: Applies the divide-and-conquer strategy, but uses optimisation techniques to find the ‘best’ division locally. Might take a while for event logs containing > 30 activities. For more information, 

\subsubsection{Inductive Miner for Incomplete Event Logs}
\label{sec:inductive-miner-incomplete}
\cite{weidlich2012profiles}
% 3)Inductive Miner - exhaustive K-successor: Applies the divide-and-conquer strategy, but searches for the best division according to the K-successor relation, 
