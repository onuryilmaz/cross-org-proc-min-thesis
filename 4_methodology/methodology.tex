...
\theoremstyle{definition}
\begin{definition}
Recommendation generation is a function that is \textit{RecGen(O, Cluster Analysis Data, Performance Threshold)} and it returns a set of \textit{Recommendation} where
	\begin{enumerate}
	  \item $O$ is identifier for organization,
	  \item \textit{Cluster Analysis Data} is result of cluster analysis stage,
	  \item \textit{Performance Threshold} is a real number between 0 and 1.
	\end{enumerate}
\end{definition}
...
RecommendationGeneration(O1, Cluster Analysis Data,  Performance Threshold)
Recommendations ={}
-> Get cluster of O1,
-> For each performance indicator in cluster analysis data (Get start and end activity)
-> Get cluster mean - cluster difference
-> if threshold > 

--- For each other cluster X 
----- For each other organizaiton in cluster X
------- Check for mismatch patterns with Start Act, End Act add to Mismatch Pattern)
return recommendation(O,A)

This stage of methodology can be visualized as gathering inputs of mismatch pattern data and cluster analysis data in Figure~\ref{fig:mismatch-pattern-analysis-blackbox}. In addition, a performance difference threshold is gathered from the user to specify how much difference between clusters is necessary to check for mismatch patterns and generate recommendations. This stage generates recommendation data for each organization which contains a set of mismatch patterns.
\todo{çiz}
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{4_methodology/mismatch-pattern-analysis-blackbox}
...
\begin{enumerate}
	...
	\item Draw Architectural concept
	\item Add screenshots for each step [Missing: Recommendation Generation step screenshots]
\end{enumerate}