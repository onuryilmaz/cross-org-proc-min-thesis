...
\begin{enumerate}
	...
	\item $Cluster\ Centroids$ is a set of tuple $(Cluster_{j}, Centroids)$ where $Centroids$ is a set of tuples $(Type, A_{start}, A_{end}, Value)$ where
		\begin{enumerate}
...
\label{alg:mismatch}
\begin{algorithmic}[1]
\Procedure {MismatchPatternAnalyzer}{$O$,$O'$,$A_{start}$,$A_{end}$}
    \State $MismatchPatterns \leftarrow \{\}$ 
    \State $MismatchPatterns$ \leftarrow  SkippedActivityAnalyzer($O$,$A_{start}$,$A_{end}$)
    \State $MismatchPatterns$ \leftarrow  RefinedActivityAnalyzer($O$,$O'$,$A_{start}$,$A_{end}$)
    \State $MismatchPatterns$ \leftarrow  DifferentMomentsAnalyzer($O$,$O'$,$A_{start}$,$A_{end}$)
    \State $MismatchPatterns$ \leftarrow  DifferentConditionsAnalyzer($O$,$O'$,$A_{start}$,$A_{end}$)
    \State $MismatchPatterns$ \leftarrow  DifferentDependencysAnalyzer($O$,$O'$,$A_{start}$,$A_{end}$)
    \State $MismatchPatterns$ \leftarrow  AdditionalDependencysAnalyzer($O$,$O'$,$A_{start}$,$A_{end}$)
    \State  \textbf{return} $MismatchPatterns$
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
Algorithm of recommendation generation function is based on the idea of checking other clusters for a significant change in performance indicators, where significance is defined by the threshold provided by user. After finding significant difference, all organizations in other clusters are checked against mismatch patterns with the starting and ending activities defined in performance indicators. With this constraining, only mismatches which are located between the activities that causes high level of difference in performance indicators are analyzed. This approach is formalized in Algorithm~\ref{alg:recgen}.

\begin{algorithm}
\caption{Recommendation Generation}
\label{alg:recgen}
\begin{algorithmic}[1]
\Procedure {RecGen}{$O$, $C$, $P$}
\State $Recommendations \leftarrow \{\}$ 
\State $i \leftarrow C(Assignments(O))$ 
\ForAll{$Centroids_{i} \in C(Cluster Centroids(i)$} 
    \ForAll{$Centroid_{i} \in  Centroids_{i}$} 
        \ForAll{$Centroids_{j} \in C(Cluster Centroids(j))$}
            \ForAll{$Centroid_{j} \in  Centroids_{j}$}  
                \If{$Centroid_{i}(A_{start}) = Centroid_{j}(A_{start})  Centroid_{i}(A_{end}) = Centroid_{j}(A_{end})$} 
                    \If{ ($\left |  Centroid_{i}(Value) -  Centroid_{j}(Value)\right | \div Centroid_{i}(Value)) \geq P$  }
                        \State $A_{start} \leftarrow Centroid_{j}(A_{start})$
                       \State $A_{end} \leftarrow Centroid_{j}(A_{end})$
                        \State $MismatchPatterns \leftarrow \{\}$
                           \ForAll{$O' \in C(Assignments(j))$}
                            \State $MismatchPatterns$ \leftarrow  MismatchPatternAnalyzer($O$,$O'$,$A_{start}$,$A_{end}$)
                        \EndFor
                        \State  $Recommendations$ \leftarrow  Recommendation($O$,$A_{start}$,$A_{end}$, $MismatchPatterns$)
                    \EndIf
                 \EndIf
            \EndFor
        \EndFor
    \EndFor
\EndFor
\State  \textbf{return} $Recommendations$
\EndProcedure
\end{algorithmic}
\end{algorithm}
