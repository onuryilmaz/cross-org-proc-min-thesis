...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.4\linewidth]{4_methodology/information-screen}
  \caption{Information Screen}
  \label{fig:information-screen}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.4\linewidth]{4_methodology/organization-name-screen}
  \caption{Organization Naming Screen}
  \label{fig:organization-name-screen}
\end{subfigure}
\caption{Starting Screens of Cross-Organizational Process Miner Plugin}
\label{fig:starting-screens}
\end{figure}

In the stage of process mining, for each event log or namely organization, user is required to provide a noise threshold and naming convention which is illustrated in Figure~\ref{fig:proc-miner-threshold-screen}. Following this, the last user input is required for matching activity names in the event logs to process models as shown in Figure~\ref{fig:automated-replayer-screen}.
\begin{figure}
\centering
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.4\linewidth]{4_methodology/proc-miner-threshold-screen}
  \caption{Process Miner Screen}
  \label{fig:proc-miner-threshold-screen}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=.4\linewidth]{4_methodology/automated-replayer-screen}
  \caption{Automated Replayer Screen}
  \label{fig:automated-replayer-screen}
\end{subfigure}
\caption{Process Mining and Automated Replayer Screens of Cross-Organizational Process Miner Plugin}
\label{fig:mining-and-replayer-screens}
\end{figure}

Finally, recommendation outputs are presented to the end user. Recommendations are visualized in two ways to create comparable outputs with and without performance clustering. When the user selects to check mismatch patterns without performance clustering, an organization should be selected at the left top of the screen in Fig XX. For the selected organization, all other organizations are compared and mismatch patterns are listed in separate tabs as shown in the main panel of Fig XX. Main idea behind adding this visualization is to provide an insight to user that how much mismatch patterns can be discovered between organizations.

w/o clustering screen + annotated
